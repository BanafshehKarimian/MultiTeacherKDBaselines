#Source code: https://github.com/Rorozhl/CA-MKD
...
from helper.loops_CAMKD import train_distill_multi_teacher as train, validate, validate_multi
...
def parse_option():
    ...
    # dataset
    parser.add_argument('--dataset', type=str, default='cifar100', choices=['pcam', 'cifar100', 'imagenet', 'tinyimagenet', 'dogs', 'cub_200_2011', 'mit67'], help='dataset')
    ...
    opt.model_name = model_name_template.format(opt.arch, opt.dataset, opt.distill, 
...
def load_teacher_list(n_cls, opt):
    ...
    teacher_model_list = [load_teacher(teacher_model_path_dict[model_name], n_cls, model_t, opt)
                          for (model_name, model_t) in zip(opt.teacher_name_list, opt.teacher_name_list)]
    print('==> done')
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    model_s = model_dict[opt.arch](**module_args)
    ...
    if opt.dataset == 'cifar100':
        if opt.distill in ['crd']:
            train_loader, val_loader, test_loader, n_data = get_cifar100_dataloaders_sample(data_folder=opt.dataset,batch_size=opt.batch_size,
                                                                               num_workers=opt.num_workers,
                                                                               k=opt.nce_k,
                                                                               mode=opt.mode)
        else:
            train_loader, val_loader, test_loader = get_cifar100_dataloaders(data_folder=opt.dataset,batch_size=opt.batch_size,
                                                                       num_workers=opt.num_workers)
    else:
        raise NotImplementedError(opt.dataset)

    if not opt.multiprocessing_distributed or opt.rank % ngpus_per_node == 0:
        logger = set_logger(opt.tb_folder+"/log.txt")
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        if not opt.multiprocessing_distributed or opt.rank % ngpus_per_node == 0:
            ...
            logger.info(f'train_acc: {train_acc}, {epoch}')
            logger.info(f'train_loss: {train_loss}, {epoch}')
        ...
        if not opt.multiprocessing_distributed or opt.rank % ngpus_per_node == 0:
            ...
            logger.info(f'test_acc: {test_acc}, {epoch}')
            logger.info(f'test_loss: {test_loss}, {epoch}')
            logger.info(f'test_acc_top5: {test_acc_top5}, {epoch}')

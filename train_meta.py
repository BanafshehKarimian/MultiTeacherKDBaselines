...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    if opt.dataset == 'cifar100':
        train_loader, val_loader = get_cifar100_dataloaders(data_folder=opt.dataset,
                                                        batch_size=opt.batch_size,
                                                        num_workers=opt.num_workers)
    elif opt.dataset == 'pcam':
        train_loader, val_loader = get_pcam_dataloaders(opt.dataset, batch_size=opt.batch_size, num_workers=opt.num_workers)
    ...
    if not opt.multiprocessing_distributed or opt.rank % ngpus_per_node == 0:
        ...
        opt.logger.info('best accuracy: {}'.format(best_acc))
